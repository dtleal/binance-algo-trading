"""VWAPPullback V2 — R-multiple trailing stop.

Identical to VWAPPullbackBot except the fixed TP is replaced by a
dynamic SL that advances through R-multiple milestones:

  < 1R profit  →  SL stays at entry − R  (original)
  ≥ 1R         →  SL moves to entry − 0.5R
  ≥ 2R         →  SL moves to breakeven (entry)
  ≥ 2.5R       →  SL moves to entry + 0.5R
  ≥ 3R         →  SL moves to entry + R
  … and so on (+0.5R per +0.5R achieved)

Each milestone advance cancels the current Binance algo SL order and
places a new one at the updated trigger price.  EOD close still exists
as the fallback exit.
"""
...
logger = logging.getLogger("trader.pullback_v2")
...
class VWAPPullbackBotV2:
    """VWAPPullback bot with R-multiple trailing stop (no fixed TP)."""
    ...
    def __init__(
        ...
        # Trailing SL state
        self._r_value: float = 0.0       # R in price terms (abs distance entry→initial_sl)
        self._sl_milestone: int = 0      # highest milestone reached (0 = original SL)
        ...
        self._reg_key = f"{self.symbol}:pullback_v2"
    ...
    def _setup_logging(self):
        ...
        log_file = LOG_DIR / f"pullback_v2_{self.symbol}_{date_str}.log"
        ...
        bot_logger = logging.getLogger(f"trader.pullback_v2.{self.symbol}")
    ...
    def _seed_indicators(self):
        ...
        limit = min(self._ema.period + 50, 1500)
        url = (
    ...
    def _check_startup_position(self):
        ...
        self._sl_milestone = 0  # Conservative: assume no milestone reached on restart
    ...
    async def run(self):
        ...
        logger.info(f"{BOLD}{prefix}VWAPPullback V2 Bot (Trailing SL) — {self.symbol}{RESET}")
        logger.info(
        ...
        self._heartbeat_task = asyncio.create_task(
            _registry.heartbeat_loop(
                self._reg_key,
                {"symbol": self.symbol, "strategy": "pullback_v2", "state": self._state.name, "dry_run": self.dry_run},
            )
        )
    ...
    def _on_kline(self, data):
        ...
        if self._state == _State.SCANNING:
            ...
            _registry.update(self._reg_key, {
                "symbol": self.symbol, "strategy": "pullback_v2",
            ...
            self._emit({
                "type": "candle", "symbol": self.symbol, "strategy": "pullback_v2",
            ...
            # Check R-multiple trailing SL on each closed candle
    ...
    def _milestone_sl_delta(self, best_r: float) -> tuple[int, float]:
        """Return (milestone_level, sl_delta_from_entry) for the given best_r."""
        R = self._r_value
        if best_r < 1.0:
            return 0, -R
        if best_r < 2.0:
            return 1, -0.5 * R
        steps = int((best_r - 2.0) / 0.5)
        return 2 + steps, steps * 0.5 * R

    def _check_trailing_sl(self, high: float, low: float):
        """Check if a new R-multiple milestone was reached and update SL accordingly."""
        ...
        if best_r <= 0:
            return  # Price hasn't moved in our favour

        new_milestone, sl_delta = self._milestone_sl_delta(best_r)

        if new_milestone <= self._sl_milestone:
            return  # No advancement — nothing to do

        # Milestone advanced — compute new SL price
        if self._direction == "long":
        ...
        if new_milestone == 0:
            ...
            label = f"+{(new_milestone - 2) * 0.5:.1f}R"
        ...
        # Cancel existing SL algo order and place new one
        try:
            ...
            # Revert to avoid state inconsistency
    ...
    async def _enter_position(self, direction: str, entry_price: float):
        ...
        if self.dry_run:
            ...
            self._emit({"type": "order", "symbol": self.symbol, "strategy": "pullback_v2",
        ...
        try:
            ...
            # Place only SL order (no TP — trailing logic handles exits)
