"""PDHL (Previous Day High/Low Rejection) bot — R-multiple trailing stop, no fixed TP."""
...
logger = logging.getLogger("trader.pdhl")
...
class PDHLBot:
    """Previous Day High/Low Rejection bot with R-multiple trailing stop (no fixed TP)."""
    ...
    def _setup_logging(self):
        ...
        log_file = LOG_DIR / f"pdhl_{self.symbol}_{date_str}.log"
        ...
        bot_logger = logging.getLogger(f"trader.pdhl.{self.symbol}")
    ...
    # ------------------------------------------------------------------
    # Indicator seeding (seed yesterday's PDH/PDL + today's running H/L)
    # ------------------------------------------------------------------
    ...
    def _seed_indicators(self):
        ...
        # Fetch ~25 hours of 1m bars to capture yesterday + today
        url = (
            f"https://fapi.binance.com/fapi/v1/klines"
            f"?symbol={self.symbol}&interval=1m&limit=1500"
        )
        logger.info("Seeding PDH/PDL from historical 1m candles...")

        try:
            with urllib.request.urlopen(url, timeout=10) as resp:
                klines = json.loads(resp.read())
        except Exception as e:
            logger.info(f"{YELLOW}Could not fetch historical klines: {e} — PDHL will need 1 full day before trading{RESET}")
            return
        ...
        for k in klines:
            ...
            elif day_ordinal == today_ordinal - 1:
                if h > yesterday_high:
                    yesterday_high = h
                if l < yesterday_low:
                    yesterday_low = l
        ...
        if yesterday_high > 0:
            self._signal._pdh = yesterday_high
            self._signal._pdl = yesterday_low
            logger.info(
                f"PDH={yesterday_high:.{pd}f} PDL={yesterday_low:.{pd}f} "
                f"(from yesterday)"
            )
        else:
            logger.info(f"{YELLOW}No yesterday data — PDHL will trade from day 2 onwards{RESET}")

        if today_high > 0:
            self._signal._today_high = today_high
            self._signal._today_low = today_low
            logger.info(
                f"Today H={today_high:.{pd}f} L={today_low:.{pd}f} "
                f"(running)"
            )
    ...
    async def run(self):
        ...
        if not self.dry_run:
            notify_bot_started(self.symbol, "PDHL", self.interval, self.leverage, self.pos_size_pct)
        ...
        try:
            ...
            if connection:
                try:
                    ...
                    pass
            if not self.dry_run:
                notify_bot_stopped(self.symbol, "PDHL")
    ...
    def _on_kline(self, data):
        ...
        prefix = "[DRY-RUN] " if self.dry_run else ""
        pd = self._price_decimals
        pdh = self._signal._pdh
        pdl = self._signal._pdl

        if self._state == _State.SCANNING:
            signal = self._signal.on_candle(c, h, l, minute_of_day)

            if pdh is not None:
                levels_label = f"PDH={pdh:.{pd}f} PDL={pdl:.{pd}f}"
            else:
                levels_label = "PDH/PDL: waiting for first full day"

            _registry.update(self._reg_key, {
                "symbol": self.symbol, "strategy": "pdhl",
                "state": self._state.name, "price": c,
                "pdh": pdh, "pdl": pdl,
                "testing_pdh": self._signal._testing_pdh,
                "testing_pdl": self._signal._testing_pdl,
            ...
            logger.info(
                ...
                f"{levels_label} | "
                f"test_pdh={self._signal._testing_pdh}({self._signal._pdh_conf}) "
                f"test_pdl={self._signal._testing_pdl}({self._signal._pdl_conf}) | "
                f"trades={self._signal.trades_today}/{self._signal.max_trades_per_day} | SCANNING"
            )
            if signal == "ENTER_LONG":
                logger.info(f"{BOLD}{GREEN}{prefix}SIGNAL: ENTER_LONG @ {c:.{pd}f} (PDL rejection at {pdl:.{pd}f}){RESET}")
                if not self.dry_run:
                    notify_signal(self.symbol, "long", round(c, self._price_decimals), "PDHL")
                self._emit({"type": "signal", "symbol": self.symbol, "strategy": "pdhl",
                            "direction": "long", "price": c, "ts": ts})
                asyncio.get_event_loop().create_task(self._enter_position("long", c))
            elif signal == "ENTER_SHORT":
                logger.info(f"{BOLD}{RED}{prefix}SIGNAL: ENTER_SHORT @ {c:.{pd}f} (PDH rejection at {pdh:.{pd}f}){RESET}")
                if not self.dry_run:
                    notify_signal(self.symbol, "short", round(c, self._price_decimals), "PDHL")
                self._emit({"type": "signal", "symbol": self.symbol, "strategy": "pdhl",
    ...
    def _check_daily_reset(self, day_ordinal: int):
        ...
        if first_candle:
            return
        pdh_new = self._signal._today_high
        pdl_new = self._signal._today_low
        logger.info(
            f"{BOLD}--- New UTC day (ordinal {day_ordinal}) — "
            f"PDH={pdh_new:.{self._price_decimals}f if pdh_new else '?'} "
            f"PDL={pdl_new:.{self._price_decimals}f if pdl_new else '?'} ---{RESET}"
        )
    ...
    async def _enter_position(self, direction: str, entry_price: float):
        ...
        try:
            ...
            notify_position_opened(
                ...
                self._sl_price, self._tp_price or None, executed_qty, self.leverage,
            )
        ...
        for pos in actual.a.P:
            ...
            if pa == 0.0 and self._state == _State.IN_POSITION:
                ...
                if self._monitor_task and not self._monitor_task.done():
                    self._monitor_task.cancel()
                self._emit({"type": "position_closed", "symbol": self.symbol,
                            "strategy": "pdhl", "reason": close_reason})
