...
use guard_sweep_common::{
    Candle, ENTRY_CUTOFF, ENTRY_START, Entry, evaluate_with_guards_stats, f2,
    find_entries_pullback, group_by_day, load_csv, precompute,
};

const TP_VALUES: &[f64] = &[0.005, 0.01, 0.015, 0.02, 0.03];
const SL_VALUES: &[f64] = &[0.004, 0.006, 0.008, 0.01, 0.015, 0.02];
const POS_SIZE_VALUES: &[f64] = &[0.10, 0.20];
const MAX_HOLD_VALUES: &[u16] = &[15, 30, 60];
const VWAP_DIST_VALUES: &[f64] = &[0.0, 0.02, 0.03, 0.05];
const TIME_STOP_VALUES: &[u16] = &[20, 40, 60];
const TIME_STOP_PROGRESS_VALUES: &[f64] = &[0.0];
const ADVERSE_EXIT_BARS_VALUES: &[usize] = &[0, 3];
const ADVERSE_BODY_MIN_VALUES: &[f64] = &[0.20];

// Intraday-focused grid: tighter targets, shorter holds, more daily rotation.
const MIN_BARS_VALUES: &[usize] = &[2, 3, 5];
const CONFIRM_BARS_VALUES: &[usize] = &[0, 1];
const VWAP_PROX_VALUES: &[f64] = &[0.001, 0.002, 0.003];
const ENTRY_WINDOWS: &[(u16, u16)] = &[(60, 1320), (360, 1080)];
const FOCUSED_VWAP_WINDOWS: &[(usize, u32)] = &[(2, 10), (3, 20), (4, 30)];
const FOCUSED_EMA_PERIODS: &[(usize, usize)] = &[(0, 100), (1, 200)];
const MAX_TRADES_PER_DAY_VALUES: &[usize] = &[2, 4];
...
fn window_label(start: u16, cutoff: u16) -> &'static str {
    if start == ENTRY_START && cutoff == ENTRY_CUTOFF {
        "01-22"
    } else {
        "06-18"
    }
...
fn ph() {
    println!(
        "  {:>4} {:>4} {:>4} {:>3} {:>2} {:>5} {:>4} {:>3} {:>3} {:>4} {:>4} {:>5} {:>4} {:>4} {:>3} {:>4} {:>4} {:>3} {:>3} {:>3} {:>5} {:>5} {:>5} {:>8} {:>8} {:>6} {:>4}",
        "TP", "SL", "R:R", "bar", "cf", "wndw", "prox", "vw", "ema", "mtd", "hold", "vds", "tstp", "tprg", "adv", "adb", "ps", "trd", "win", "eod", "eod%", "avgH", "win%", "ret%", "final$", "dd%", "mcl"
    );
}

fn pr(r: &RunResult) {
    let vds = if r.vwap_dist_stop == 0.0 {
        "-".to_string()
    } else {
        f2(r.vwap_dist_stop)
    };
    let tstop = if r.time_stop_minutes == 0 {
        "-".to_string()
    } else {
        r.time_stop_minutes.to_string()
    };
    let tprg = if r.time_stop_minutes == 0 {
        "-".to_string()
    } else {
        f2(r.time_stop_min_progress_pct)
    };
    let adv = if r.adverse_exit_bars == 0 {
        "-".to_string()
    } else {
        r.adverse_exit_bars.to_string()
    };
    let adb = if r.adverse_exit_bars == 0 {
        "-".to_string()
    } else {
        f2(r.adverse_body_min_pct)
    };
    println!(
        "  {:>4.1} {:>4.1} {:>4.1} {:>3} {:>2} {:>5} {:>4.1} {:>3} {:>3} {:>4} {:>4} {:>5} {:>4} {:>4} {:>3} {:>4} {:>4.0} {:>3} {:>3} {:>3} {:>4.1} {:>5.1} {:>4.1}% {:>7.2}% {:>8.2} {:>5.2}% {:>4}",
        r.tp_pct,
        r.sl_pct,
        r.rr_ratio,
        r.min_bars,
...
fn main() {
    ...
    let csv_file = if args.len() > 1 {
        ...
        "../data/klines/ETHUSDT_1m_klines.csv"
    };
    ...
    println!("  Dedicated sweep: VWAPPullback intraday research grid\n");
    ...
    println!(
        "\nPhase 2 — evaluating {} combinations (parallel)...",
        entry_sets.len() * combos_per_set
    );
                                        ...
                                        for &aeb in ADVERSE_EXIT_BARS_VALUES {
                                            for &abm in ADVERSE_BODY_MIN_VALUES {
                                                ...
                                                rows.push(RunResult {
                                                    ...
                                                    pos_size_pct: ps * 100.0,
                                                    trades: n,
                                                    avg_trades_per_day: ((n as f64 / total_days) * 100.0).round() / 100.0,
                                                    wins: stats.wins,
                                                    losses: stats.losses,
                                                    eods: stats.eods,
                                                    eod_ratio_pct: (stats.eod_ratio_pct * 10.0).round() / 10.0,
                                                    avg_hold_minutes: (stats.avg_hold_minutes * 10.0).round() / 10.0,
                                                    win_rate: (stats.wins as f64 / n as f64 * 1000.0).round() / 10.0,
                                                    return_pct: ((stats.final_capital / 1000.0 - 1.0) * 10000.0).round() / 100.0,
                                                    final_capital: (stats.final_capital * 100.0).round() / 100.0,
                                                    max_dd_pct: (stats.max_dd * 10000.0).round() / 100.0,
                                                    max_consec_loss: stats.max_consec_loss,
                                                });
                                            }
                                        }
                                    }
    ...
    for r in &results {
        ...
        writer
            .write_record([
                ...
                "-".to_string(),
                "-".to_string(),
                r.max_hold.to_string(),
                if r.vwap_dist_stop == 0.0 {
                    "-".to_string()
                } else {
                    f2(r.vwap_dist_stop)
                },
                if r.time_stop_minutes == 0 {
                    "-".to_string()
                } else {
                    r.time_stop_minutes.to_string()
                },
                ...
                r.trades.to_string(),
                f2(r.avg_trades_per_day),
                r.wins.to_string(),
                r.losses.to_string(),
                r.eods.to_string(),
                f2(r.eod_ratio_pct),
                f2(r.avg_hold_minutes),
                format!("{:.1}", r.win_rate),
                f2(r.return_pct),
                f2(r.final_capital),
                f2(r.max_dd_pct),
    ...
    println!("{}", "=".repeat(180));
    println!("  TOP 20 VWAPPULLBACK INTRADAY SWEEP BY RETURN %");
    println!("{}", "=".repeat(180));
    ph();
    ...
    println!("\n{}", "=".repeat(180));
    println!("  TOP 20 VWAPPULLBACK INTRADAY SWEEP RISK-ADJUSTED");
    println!("{}", "=".repeat(180));
    ph();
    ...
    for &mh in MAX_HOLD_VALUES {
        ...
        println!("  max_hold={:>3}m: avg_ret={:+6.2}% (n={})", mh, a, s.len());
    }
    for &tsm in TIME_STOP_VALUES {
        let label = if tsm == 0 {
            "off".to_string()
    ...
    for &mtd in MAX_TRADES_PER_DAY_VALUES {
        let s: Vec<&&RunResult> = active.iter().filter(|r| r.max_trades_per_day == mtd).collect();
        let a = s.iter().map(|r| r.return_pct).sum::<f64>() / s.len().max(1) as f64;
        println!("  max_trades_day={:>1}: avg_ret={:+6.2}% (n={})", mtd, a, s.len());
    }
    let avg_eod = active.iter().map(|r| r.eod_ratio_pct).sum::<f64>() / active.len().max(1) as f64;
    let avg_hold = active.iter().map(|r| r.avg_hold_minutes).sum::<f64>() / active.len().max(1) as f64;
    println!("  avg_eod_ratio={:>6.2}% (all active)", avg_eod);
    println!("  avg_hold_minutes={:>6.2} (all active)", avg_hold);
}
